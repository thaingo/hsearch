...
var NameValue = new Class({
    ...
    reset: function() {
    	var totalEl = this.names.length;
        this.names.splice(0, totalEl );
        this.values.splice(0, totalEl );
    },
    ...
    remove: function(aName) {
		var totalEl,theName,i;
    	totalEl = this.names.length;
		theName = null;
        for (i = 0; i < totalEl; i++) {
    ...
    lilo: function(watermark) {
    	var totalEl = this.names.length;
    	if ( totalEl > watermark ) this.removeAtIndex(0);
    },

    getValue: function(aName) {
        return this.spot( aName, false );
    },

    getAndRemove: function(aName) {
        return this.spot( aName, true );
    },

    isExists: function(aName) {
        var theValue = this.spot( aName, false );
    ...
    spot: function(aName, isDelete) {
    	var totalEl,theName,theValue,i;
        ...
        for (i = 0; i < totalEl; ++i) {
			theName = this.names[i];
			theValue = this.values[i];
			if ( aName == theName ) {
			    if ( isDelete ) this.removeAtIndex (i);
			    break;
			}
			theName = null;
			theValue = null;
...
var StringBuilder = new Class({
	...
	clear: function() {
    	var totalEl = this.strings.length;
        this.strings.splice(0, totalEl );
	},
	toString: function() {
...
function setupParameters() {
	...
	if(window.location.search) {
		var paramArray,length,index,param,name,value;

		paramArray = window.location.search.substr(1).split('&');
		length = -1;
		if ( paramArray ) length = paramArray.length;
		for (index = 0;index <length; index++ ) {
			param = paramArray[index].split('=');
			name = param[0];
			value = (typeof param[1] == "string") ? 
				decodeURIComponent(param[1].replace(/\\+/g, " ")) : null;
...
function getParameter(key) {
	if(typeof window.location.parameters == "undefined") setupParameters();
	return window.location.parameters[key];
}
...
function dragContainerInit(el){
    var dragContainerOptions = {
	    ...
	    onStart: function(){
		    ...
		    fadeIn.custom(1,.5);
		    zLevel++; 
		    el.parentNode.style.zIndex = zLevel;
	    }.bind(this),
	    onComplete: function(){
		    var fadeIn = new fx.Opacity(el.parentNode, {duration:300});
		    fadeIn.custom(.5,1);
...
Element.extend({
    ...
    isNumber : function() {
        return this.findValue().isNumber();
    },
    isHint : function() {
        return this.findValue().isHint();
    },
    isNotEmpty : function() {
...
var PROCESS_XML,DOWNLOAD_URL;
PROCESS_XML = 'service.html';
DOWNLOAD_URL = 'filedownload.html';

var Request = new Class({
    initialize: function() {
    	this.reqNo = 0;
    	this.requestCache = new NameValue();
    	this.sucessF = sucessF;
    	this.failureF = failureF;
    },

    beforeCall: function(url, model, displayArea, errorArea, callback, notify) {

    	if( this.reqNo == 10 ) this.reqNo = 0;

    	var key = this.reqNo++ ;
    	this.requestCache.remove(key);
    	this.requestCache.add( key , 
    		{ 
    		    MODEL: model, 
    		    DISPLAY: displayArea, 
    		    ERROR: errorArea, 
    	...
    	displayArea.innerHTML = '<img src="ajax.gif"> Please wait...';
    	return { URL: url, REQNO: key};
    },
    
    call: function(url, model, displayArea, errorArea, callback ) {
    	var callInfo = 
    	    this.beforeCall(url, model, displayArea, 
    	    errorArea, callback, null);
    	this.onCall(model, callInfo);
    },

    callAndNotify: function(url, model, displayArea, errorArea, callback, notify ) {
    	var callInfo = this.beforeCall(url, model, displayArea, 
    	    errorArea, callback, notify);
    	    
    	this.onCall(model, callInfo);
    },
            
    onGet: function(url, displayArea, errorArea, getSucess, getFailure ) {
    	errorArea.innerHTML = '<img src="ajax.gif"> Please wait...';
        new Ajax(url,{onComplete: getSucess, onFailure: getFailure}).request();
    },

    
    onCall: function(model,callInfo) {
    	if ( null == model ) model = new Object();
    	model.stamp = callInfo.REQNO;
    	new Ajax(callInfo.URL,{postBody:model,onComplete: this.sucessF, onFailure: this.failureF}).request();
    	this.afterCall(callInfo);
    	return false;
    },

    afterCall: function(callInfo) {
    	callInfo.URL = null;
    	callInfo.REQNO = null;
    }
...
/**----------------------------------------------------------- */
function sucessF(response) {
	httpResponse.consume(response,true);
}

function failureF(response) {
	httpResponse.consume(response,false);
}
...
var Response = new Class({
    ...
    initialize: function() {
    	this.SVR_ERROR_MSG = "Server processing failure.\n";
    },

    beforeConsume: function(response) {
    	
    	var responseL,msgId,req,resIndex;
    	
    	//Process the response
    	if ( response == null ) return;
    	responseL = response.length;
    	if ( responseL == 0 ) return;
    	
		msgId = 0;
		req = null;
		resIndex = response.indexOf("<result msgid=\"");
		if ( resIndex >= 0 ) {
		  msgId = response.substr( resIndex + 15, 3);
		  msgId = msgId.replace("\"","");
		  req = httpRequest.requestCache.getAndRemove(msgId);
		}
		if ( req == null ) req = new Object(); //In case no message id
		req.BODY = response;
				
		//Now return the request object.
		return req;
    },
    
    consume: function(response,isSucess) {

		var res,errorIndex;
    	res = this.beforeConsume(response);
		if ( res != null ) {
			if ( isSucess) {
				errorIndex = response.indexOf("<error>");
				if ( errorIndex > 0 ) {
				    if ( res.DISPLAY ) res.DISPLAY.innerHTML = "Failure:" + response;
				}
				else {
    		...
    		this.afterConsume(res);
		} else {
			//alert("No Response.");
		}
    },

    afterConsume: function(res) {
		if ( ! res ) return;
    	if ( res.CALLBACK ) res.CALLBACK = null;
		if ( res.MODEL ) res.MODEL = null;
		if ( res.DISPLAY ) res.DISPLAY = null;
...
function normalizeFT(freeText) {
	freeText = freeText.replace(/&/g,"-am-");
	freeText = freeText.replace(/=/g,"-eq-");
	freeText = freeText.replace(/\+/g,"-pl-");
	freeText = freeText.replace(/\n/g,"-nl-");
	freeText = freeText.replace(/:/g,"-co-");
	freeText = freeText.replace(/~/g,"-tl-");
	freeText = freeText.replace(/</g,"-lt-");
	freeText = freeText.replace(/>/g,"-gt-");
	return freeText;
}

function reverseToFT(freeText) {
	freeText = freeText.replace(/-am-/g,"&");
	freeText = freeText.replace(/-eq-/g,"=");
	freeText = freeText.replace(/-pl-/g,"\+");
	freeText = freeText.replace(/-nl-/g,"\n");
	freeText = freeText.replace(/-co-/g,":");
	freeText = freeText.replace(/-tl-/g,"~");
	freeText = freeText.replace(/-lt-/g,"<");
	freeText = freeText.replace(/-gt-/g,">");
	freeText = freeText.replace(/\\\'/g,"'");
	freeText = freeText.replace(/&apos;/g,"'");
	freeText = freeText.replace(/&quot;/g,"\"");
	return freeText;
}    
function reverseToHtml(freeText) {
	freeText = freeText.replace(/-am-/g,"&amp;");
	freeText = freeText.replace(/-eq-/g,"=");
	freeText = freeText.replace(/-pl-/g,"\+");
	freeText = freeText.replace(/-nl-/g,"<BR />");
	freeText = freeText.replace(/-co-/g,":");
	freeText = freeText.replace(/-tl-/g,"~");
	freeText = freeText.replace(/-lt-/g,"&lt;");
	freeText = freeText.replace(/-gt-/g,"&gt;");
...
function ripXmlSingularTag(docXml, nodeName) {
        
    var tagNode = docXml.getElementsByTagName(nodeName);
    if ( null != tagNode && tagNode.length == 1 ) {
        if (null != tagNode[0] && tagNode[0].childNodes.length == 1)
            return tagNode[0].childNodes[0].nodeValue;
        else return "";
    } else {
        return "";
    }       
...
function blankF(res){}
function routerF(res){
    res.NOTIFY(res);
}/**----------------------------------------------------------- */
/** Load general basic files */
/**----------------------------------------------------------- */
...
loadJsCssFile("/searchcherry/static/css/bw.css", "css");var SensorContent = new Class({
    initialize: function() {
    },

    /**
     * For a give id, load the html page
     */
    fetchContentById: function(id, container, error, onSucessF) {
	    var data = new Object();
	    data.sensor="sensor.content";
	    data.action="GET_CONTENT_BYID";
	    if ( id ) data.id = id;
        else return;
        
        httpRequest.callAndNotify(PROCESS_XML, data, container, error, 
            SensorContent_ContentLoading,onSucessF );
    },
    
    loadContent: function(res) {
	    if ( null != res.ERROR ) res.ERROR.innerHTML = 
	    	"Content is found. Please wait, Parsing now..";
	    // var model, docXml, structureNode;
	    var model, docXml;
	    docXml = loadXMLDoc(res.BODY);
	    model = res.MODEL;
	    
	    // Firefox load string is limited to 4098 characters
	    /**
	    if ( docXml ) {
	        model.id = ripXmlSingularTag(docXml, "id");
	        model.title = ripXmlSingularTag(docXml, "title");
	        model.desc = ripXmlSingularTag(docXml, "desc");
	        structureNode = docXml.getElementsByTagName("structures");
	    } else {
	        res.BODY = "";
	    }
	    if ( structureNode && structureNode.length == 1)
	        res.BODY = ripXmlSingularTag(structureNode[0], "val");
	    else res.BODY = "";
	    */
	    ...
	    if ( docXml ) {
	    	model.id = ripXmlSingularTag(docXml, "id");
	    	model.title = ripXmlSingularTag(docXml, "title");
	        res.BODY = res.BODY.substring ( 
	            res.BODY.indexOf("<val>") + 5, res.BODY.indexOf("</val>") );
	    } else {
	    	res.BODY = "";
	    }
    ...
    /**
     * For a give title, load the top matching page
     */
    fetchHtmlByPage: function(page, container, error, onSucessF) {
	    var data = new Object();
	    data.sensor="sensor.content";
	    data.action="SEARCH";
	    data.fl="id";
	    if ( page ) data.q= 
	    	"dna:html AND state:active AND tenant:" + cookieMgr.readCookie("TENANT") + " AND title:\"" + page + "\"";
        else return;
        
        httpRequest.callAndNotify(PROCESS_XML, data, container, error, 
            SensorContent_SearchLoading,onSucessF );
    },
    
    /**
     * Loads the search scanned pages, both via title or via id
     */
    loadUnstructuredHtml: function(res) {
		var pageXml, pageId,cL;
	    pageXml = loadXMLDoc(res.BODY);
		cL = pageXml.getElementsByTagName("id");
		if ( null != cL && cL.length == 1 ) {
		    pageId = cL[0].childNodes[0].nodeValue;
            this.fetchContentById(pageId, res.DISPLAY, res.ERROR, res.NOTIFY);
		} else {
			res.ERROR.innerHTML = "Content is not available yet.<BR />You can try after sometime.";
		}
    },
    
    renderHtml: function(res) {

        // ******** Firefox supports till 4096 characters ******** 
        /**
        var docXml, recordNode, htmlText;
        docXml  = loadXMLDoc(res.BODY);
        recordNode = docXml.getElementsByTagName("r");
        htmlText = ripXmlSingularTag( recordNode[0], "src");
        */
        var htmlText; 
        if ( res.BODY.length > 44 ) {
            htmlText = res.BODY.substring ( 20, res.BODY.length - 22);
            htmlText = reverseToFT(htmlText);
        } else htmlText = "";
        
        htmlText = reverseToFT(htmlText);
        res.DISPLAY.innerHTML = htmlText;
        res.BODY = htmlText;
    },
    
    /**
     * Remove the HTML by Id
     */
    removeHtmlById: function(id, container, error, onSucessF) {
	    var data = new Object();
	    data.sensor="sensor.content";
	    data.action="PROCES_WORKFLOW";
		data.dna="html";
		data.operation="Remove";
	    data.id = id;
	    httpRequest.callAndNotify(PROCESS_XML, data, 
	    	container, error, routerF,onSucessF);
    },
    
    /**
     * Save the HTML page. If the page is already present modify it.
     * On sucessful saving notify the caller executing the call back function. 
     */
    saveHtml: function(id, pageName,conent,container, error, onSucessF) {
		error.focus();
		if ( pageName == "null" || pageName == "" ) {
			error.innerHTML = "No page name specified.";
			return;
		}
			
		error.innerHTML = "Saving content.. Please wait";
		var data = new Object();
		data.sensor="sensor.content";
		data.action="PROCES_WORKFLOW";
		data.title=normalizeFT(pageName);
		data.stdate = new Date().getTime();
		data.dna="html";

		var bodyText,contentTop,strContent,contentButtom;
		
		bodyText = conent.replace( /(<head>(\n|.)*<\/head>)/i, ""); 
		bodyText = bodyText.replace( /(<style>(\n|.)*<\/style>)|(<script>(\n|.)*<\/script>)|(<([^>]+)>)|(<![\s\S]*?--.[ \.\t\n\r]*>)|(\&nbsp;)|([\r\n]+)/g, " ");
		bodyText=bodyText.replace(/<.*?>/g,"");
		data.desc=normalizeFT(bodyText);
		data.zT="1";
		
		contentTop = "page~<r><src>"
		strContent = normalizeFT(conent);
		contentButtom = "</src></r>";
		data.z0 = contentTop + strContent + contentButtom;
    ...
    /**
     * For a give id and timestamp, get the nearest version available
     */
    fetchHtmlByVersion: function(id, timeStamp, container, error, onSucessF) {
		if ( ! id) return;
		if ( "" == id ) return;
		var data = new Object();
		data.sensor = "sensor.content";
		data.action = "GET_STRUCTURE_BYTS";
		data.id = id;
		data.timestamp = timeStamp;
        httpRequest.callAndNotify(PROCESS_XML, data, container, error, 
            SensorContent_VersionLoading,onSucessF );
    },
    
    fetchHtmlLatest: function(id, container, error, onSucessF) {
        sensorContent.fetchContentById(id,container,error, onSucessF);
    },

    /**
     * Load a specfic version directly from the hbase. This is the latest available
     */
    loadVersion : function(res) {
    
		var error, disp, notify, model, content, when, startI, endI, pageHtml;
		...
		when = new Date();
		when.setTime(model.timestamp);
		error.innerHTML = when;
		startI = content.indexOf("<val>");
		endI = content.indexOf("</val>");
		pageHtml = content.slice(startI + 5,endI);
		pageHtml = reverseToFT(pageHtml);
		pageHtml = pageHtml.slice(20); //&lt;r&gt;&lt;src&gt;
		pageHtml = pageHtml.slice(0,pageHtml.length - 22); //&lt;/src&gt;&lt;/r&gt;
		disp.innerHTML = pageHtml;
		res.BODY = pageHtml;
		notify(res);
    },
    
    /**
     * Get all the available versions for the document
     */
    fetchHtmlRevisions: function(id, container, error, onSucessF) {
		if ( ! id ) return;
		if ( "" == id ) return;
		var data = new Object();
		data.sensor = "sensor.content";
		data.action = "GET_STRUCTURE_VERSION";
		data.field = "content:ts";
		data.id = id;
    ...
    /**
     * Attach already uploaded files to the contnet
     */
    attachFile: function(id,pA,container, error, onSucessF) {
		if ( ! id ) return;
		if ( "" == id ) return;
		if ( !pA ) return;
		...
		var data = new Object();
		data.sensor = "sensor.content";
		data.action="CONTENT_ATTACH";
		data.id = id;
		var pL = pA.length;
		data.aT=pL;
		for (var pI = 0;pI < pL; pI++ ) {
			if ( 0 == pI ) data.a0 = pA[pI];
			else if ( 1 == pI ) data.a1 = pA[pI];
			else if ( 2 == pI ) data.a2 = pA[pI];
			else if ( 3 == pI ) data.a3 = pA[pI];
			else if ( 4 == pI ) data.a4 = pA[pI];
			else if ( 5 == pI ) data.a5 = pA[pI];
			else if ( 6 == pI ) data.a6 = pA[pI];
			else if ( 7 == pI ) data.a7 = pA[pI];
			else if ( 8 == pI ) data.a8 = pA[pI];
			else if ( 9 == pI ) data.a9 = pA[pI];
			else if ( 10 == pI ) data.a10 = pA[pI];
			else if ( 11 == pI ) data.a11 = pA[pI];
			else if ( 12 == pI ) data.a12 = pA[pI];
		}			
	    httpRequest.callAndNotify(PROCESS_XML, data, 
	    	container, error, routerF,onSucessF);
	},
	
    searchTeaser: function(searchText, startPosition, pageSize, openLinkFName, container, error, onSucessF) {
        if ( ! searchText ) return;
	    var data = new Object();
	    data.sensor="sensor.content";
	    data.action = "SEARCH_TEASER";
	    data.start = startPosition;
	    data.end = pageSize;
        data.sortby = 'stdate';
        data.sortorder = "false";
	    data.openLink = openLinkFName;
	    data.q = "dna:html AND state:active AND tenant:" + cookieMgr.readCookie("TENANT") + " AND " + searchText;
        httpRequest.callAndNotify(PROCESS_XML, data, container, error, 
            SensorContent_TeaserLoading,onSucessF );
    },

    searchDnaTeaser: function(dna, searchText, startPosition, pageSize, openLinkFName, container, error, onSucessF) {
        if ( ! searchText ) return;
	    var data = new Object();
	    data.sensor="sensor.content";
	    data.action = "SEARCH_TEASER";
	    ...
	    data.end = pageSize;
	    data.openLink = openLinkFName;
	    if ( dna.length > 0 ) data.q = "dna:" + dna + " AND tenant:" + cookieMgr.readCookie("TENANT") + " AND " + searchText;
	    else data.q = "tenant:" + cookieMgr.readCookie("TENANT") + " AND " + searchText;
    ...
    loadTeaser : function(res) {
		var error, errorId, notify, model, content, disp, dispId, xmlResult;
		...
		xmlResult = loadXMLDoc(content);
		error.innerHTML = "";
		
		//Populate the cluster HTML
		var cI, cL,cT,callbackName,openLinkFN,clusterHtml,segment,cLabel,cTotal,onClickExecute;

		cL = xmlResult.getElementsByTagName("cluster");
		cT = cL.length;
		
		//Find out the callback function name to form the new link
		callbackName = "" + notify;
		callbackName = callbackName.slice(8,callbackName.indexOf("("));
		
		openLinkFN = "" + model.openLink;
		if ( openLinkFN.indexOf("(") > 0 ) openLinkFN = openLinkFN.slice(8,openLinkFN.indexOf("("));
		else openLinkFN = model.openLink;
		
		clusterHtml = "";
		for (cI=0; cI < cT ; cI++) {
			segment = cL[cI].childNodes[0];
			cLabel = reverseToFT(segment.childNodes[0].childNodes[0].nodeValue);
			cTotal = segment.childNodes[1].childNodes[0].nodeValue;
			if ( cLabel == "(Other)" ) continue;

            onClickExecute = 'sensorContent.searchTeaser("' + model.q + " AND " + cLabel  + 
                '",0,' + model.end + ',"' + openLinkFN +  
                '", $("' + dispId + '"),$("' + errorId + '"), ' + 
                callbackName + ');';
            
			clusterHtml = clusterHtml + 
			    '<span class="small"> <a href="#" onClick=\'' + 
			    onClickExecute + '\' >' + 
			    cLabel + '</a>(' + 
			    cTotal + ')</span>';
		}
		model.clusterHtml = clusterHtml;
		
		//prepare the teaser
		var docXml,i,j,dna,title,id,teaser,date,fT,fld,idVal,titleVal,docHtml,prevIndex,nextIndex,onNextExecute,onPrevExecute,nextPrev;
		
		docXml = xmlResult.getElementsByTagName("doc");
		model.docHtml = "";
		...
		for (i=0; i < docXml.length; i++) {
			...
			teaser = null;
			date = null;
			fT = docXml[i].childNodes.length;
			for ( j=0; j < fT; j++ ) {
				fld = docXml[i].childNodes[j].nodeName;
				if ( "dna" == fld ) dna = docXml[i].childNodes[j];
				else if ( "title" == fld ) title = docXml[i].childNodes[j];
				else if ( "id" == fld ) id = docXml[i].childNodes[j];
				else if ( "teaser" == fld ) teaser = docXml[i].childNodes[j];
				else if ( "date" == fld ) date = docXml[i].childNodes[j];
			}
			idVal = id.childNodes[0].nodeValue;
			if ( title.childNodes.length == 0 ) titleVal = idVal;
			else titleVal = title.childNodes[0].nodeValue;
			docHtml = '<div id="' + idVal + 
			    '" class="vbox"> <a href="#" onClick=\'' + openLinkFN + 
			    '("' + idVal + '","' + titleVal + '")\' >' + titleVal + '</a>' ;
			if ( date ) docHtml = docHtml + '<span class="stamp"><em>  ' + 
				date.childNodes[0].nodeValue + '  </em></span>';
				
			if ( teaser.childNodes.length == 0 ) {
			    docHtml = docHtml + '<br /> <span class="teaser" >' + idVal + '</span></div>';
			} else {
			    docHtml = docHtml + '<br /> <span class="teaser" >' + 
				    teaser.childNodes[0].nodeValue + '</span>' + '</div>';
            }				    
			if ( model.docHtml ) model.docHtml = model.docHtml + docHtml;
			else model.docHtml = docHtml;
		}
		model.docHtml = reverseToFT(model.docHtml);
        prevIndex = model.start - model.end;
        nextIndex = model.start + model.end;
        
        onNextExecute = 'sensorContent.searchTeaser("' + model.q  + 
            '",' + nextIndex + ',' + model.end + ',"' + openLinkFN + 
            '", $("' + dispId + '"),$("' + errorId + '"), ' + 
            callbackName + ');';
            
        onPrevExecute = 'sensorContent.searchTeaser("' + model.q  + 
            '",' + prevIndex + ',' + model.end + ',"' + openLinkFN +  
            '", $("' + dispId + '"),$("' + errorId + '"), ' + 
            callbackName + ');';
		
		nextPrev = '<div class="small"> ' ;
		if ( model.start != 0 ) //Don't show previous at start
			nextPrev = nextPrev + '<div style="float:left; padding:5px; color:#f00;">&lt;&lt; <a href="#" onClick=\'' + onPrevExecute + '\'>Previous</a></div>';
		if ( docXml.length == model.end ) //There is no more next
			nextPrev = nextPrev + '<div style="float:right; padding:5px; color:#ff0000;"><a href="#" onClick=\'' + onNextExecute + '\'>Next</a> &gt;&gt;</div>';
		model.nextPrev = nextPrev + "</div>";
		
		notify(res);
    },

    /**
     * For a give file name, load the available files
     */
    fetchFileByExtension: function(fileName, openLinkFName, container, error, onSucessF) {

	    var data = new Object();
	    data.sensor="sensor.content";
	    data.action = "FILES_LS";
	    data.matching = fileName;
	    data.openLink = openLinkFName;
        httpRequest.callAndNotify(PROCESS_XML, data, container, error, 
            SensorContent_FileLoading,onSucessF );
    },
    
    showFileList : function(res) {
		var error, notify, model, content, display,xmlResult,openLinkFN;
	    error = res.ERROR;
	    notify = res.NOTIFY;
	    model = res.MODEL;
	    content = res.BODY;
		...
		//Populate the File HTML
		var itemL,itemT,fileListingHtml,aFileHtml,pathVal,fileName,i,j,path,fT,fld, fileNameI;
		
		itemL = xmlResult.getElementsByTagName("item");
		itemT = itemL.length;
		fileListingHtml = "";
		aFileHtml = "";
		pathVal = "";
		...
		for (i=0; i < itemT; i++) {
			path = null;
			fT = itemL[i].childNodes.length;
			for ( j=0; j < fT; j++ ) {
				fld = itemL[i].childNodes[j].nodeName;
				if ( "path" == fld ) {
				    path = itemL[i].childNodes[j];
				    break;
				}
			}
			pathVal = path.childNodes[0].nodeValue;
			fileNameI = pathVal.lastIndexOf("\/");
			if ( fileNameI > -1 ) fileName = pathVal.slice(fileNameI + 1);
			aFileHtml = fileName + '&nbsp;<a href="#" onClick=\'' + openLinkFN + 
			    '("' + pathVal + '","' + fileName + '")\' >Open</a>' +
			    '&nbsp;<a href="#" onClick=\'sensorContent.removeFile("' + fileName + '","' + model.matching +
			    '","' + display.id + '","' + error.id + '","' + openLinkFN + '")\' >Remove</a><br />';
			fileListingHtml = fileListingHtml + aFileHtml;
		}
		if ( "" == fileListingHtml ) display.innerHTML = "No matching files found.";
		else display.innerHTML = fileListingHtml;
		notify(res);
    },
    
    removeFile: function(fileName, matching, container, error, openLinkFName) {

	    var data = new Object();
	    data.sensor="sensor.content";
	    data.action = "FILES_RM";
	    data.matching = fileName; //Exact file name
	    data.hint = matching; //original matching string
	    data.openLinkFName = openLinkFName;
        httpRequest.callAndNotify(PROCESS_XML, data, $(container), $(error), 
        	routerF, SensorContent_FileRemove );
    },

    removeFileSS : function(res) {
		var error, model, display;
	    error = res.ERROR;
	    model = res.MODEL;
	    display = res.DISPLAY;
	    error.innerHTML = "Sucessfully removed. Refreshing the list again.";

		sensorContent.fetchFileByExtension(model.hint, model.openLinkFName, display, error, blankF);
	},
	
    loadHtmlSnippet: function(url, displayArea, errorArea, onSucessF, onFailureF) {
       httpRequest.onGet(url, displayArea, errorArea, onSucessF , onFailureF) ;
    },

    rewriteLinks: function(_contentDiv) {
    
    	var links,linkI,linksT, aLink, aLinkText, selfLink, selfLinkT, selfLinkL,selfLinkI,htmlId;
    
        selfLinkL = new Array();
        selfLinkL[0] = 'http://' + window.location.host + 
            '/searchcherry/static/viewers/htmlviewer.html?id=';
        selfLinkL[1] = '/searchcherry/static/viewers/htmlviewer.html?id=';
        selfLinkT = selfLinkL.length;

		links = _contentDiv.getElementsByTagName('a');
		linksT = links.length;
		
		for ( linkI=0; linkI < linksT ; linkI++) {
			aLink = links[linkI];
			if ( ! aLink.href || "undefined" == aLink.href || "" == aLink.href ) continue;
			
			for (selfLinkI=0; selfLinkI < selfLinkT;  selfLinkI++)
			{
				selfLink = selfLinkL[selfLinkI];
				if ( aLink.href.indexOf(selfLink) == 0 ) {
					htmlId = aLink.href.replace(selfLink,"");
					aLink.href = "#";
					aLinkText = aLink.innerHTML;
					var funText = 'return gotoLink("' + htmlId + '",\'' + aLinkText + '\');';
                    aLink.onclick = new Function(funText);  
					break;
				}
			}
		}	
...
function SensorContent_ContentLoading(res) {
    sensorContent.loadContent(res);
}

function SensorContent_SearchLoading(res) {
    sensorContent.loadUnstructuredHtml(res);
}

function SensorContent_VersionLoading(res) {
    sensorContent.loadVersion(res);
}

function SensorContent_TeaserLoading(res) {
    sensorContent.loadTeaser(res);
}

function SensorContent_FileLoading(res) {
    sensorContent.showFileList(res);
}

function SensorContent_FileRemove(res) {
    sensorContent.removeFileSS(res);
}

/**
* Singleton instance for the content sensor
*/
var sensorContent = new SensorContent();var SensorSiteAdmin = new Class({
    initialize: function() {
    },

    /**
     * For a given role home page by role name.
     */
    getRoleByName: function(roleName, container, error, onSucessF) {
	    var data = new Object();
	    data.sensor="sensor.siteadmin";
	    data.action="TENANT_GET_ROLE";
		data.rolename = roleName;
        httpRequest.callAndNotify(PROCESS_XML, data, container, error,routerF,onSucessF);
    },
    
    saveRoleByName: function(roleName, tabXml, error, onSucessF) {
	    var data = new Object();
	    data.sensor="sensor.siteadmin";
	    data.action="TENANT_ADD_ROLE";
		data.tabcontainerxml = tabXml;
		data.rolename = roleName;
        httpRequest.callAndNotify(PROCESS_XML, data, error, error,routerF,onSucessF);
    },
    


    /**
     * For a given widget name, get the respective XML
     */
    getWidgetByName: function(widgetName, container, error, onSucessF) {
	    var data = new Object();
	    data.sensor="sensor.siteadmin";
	    data.action="TENANT_GET_WIDGET";
		data.widgetname = widgetName;
        httpRequest.callAndNotify(PROCESS_XML, data, container, error,routerF,onSucessF);
    },
    
    /**
     * Process the start widget. Calls back the given method for each tab
     */
    getStartWidget: function(container, error, onSucessF) {
	    var data = new Object();
	    data.sensor="sensor.siteadmin";
	    data.action="TENANT_GET_WIDGET";
		data.widgetname = "start";

        httpRequest.callAndNotify(PROCESS_XML, data, container, error, 
            SensorSiteAdmin_StartWidgetLoading,onSucessF );
    },
    
    /**
     * Load the start widget by iterating each indivisual elements.
     */
    loadStartWidget : function(res) {
        var notify,xmlTabs,xmlTabL,i,j,label,url,fT,fld;
        
        notify = res.NOTIFY;
		xmlTabs = loadXMLDoc(res.BODY);
		xmlTabL = xmlTabs.getElementsByTagName("tab");
		for (i=0; i < xmlTabL.length; i++) {
			label = null;
			url = null;
			fT = xmlTabL[i].childNodes.length;
			for ( j=0; j < fT; j++ ) {
				fld = xmlTabL[i].childNodes[j].nodeName;
				if ( "label" == fld ) label = xmlTabL[i].childNodes[j];
				else if ( "url" == fld ) url = xmlTabL[i].childNodes[j];
			}
			notify(label.childNodes[0].nodeValue, url.childNodes[0].nodeValue);
		}        
    },
     ...
     * Process the start widget. Calls back the given method for each tab
     */
    getAllWidgets: function(container, error, onSucessF) {
	    var data = new Object();
	    data.sensor="sensor.siteadmin";
	    data.action="TENANT_ALL_DNANAMES";

        httpRequest.callAndNotify(PROCESS_XML, data, container, error, 
            SensorSiteAdmin_AllWidgetLoading,onSucessF );
    },
    
    /**
     * Load the start widget by iterating each indivisual elements.
     */
    loadAllWidgets : function(res) {
		var notify,display,dnaL,resNode;

        notify = res.NOTIFY;
        display = res.DISPLAY;
		dnaL = loadXMLDoc(res.BODY);
		resNode = dnaL.getElementsByTagName("result");
        if ( resNode ) {
            var dnaJoin,dnaJoinSplit,dnaJoinSplitT,j,optionText,anOption;
            
            dnaJoin = resNode[0].childNodes[0].nodeValue;
            dnaJoinSplit = dnaJoin.split('~');
            dnaJoinSplitT = dnaJoinSplit.length;
	        for ( j=0; j<dnaJoinSplitT; j++ ) {
	            optionText = dnaJoinSplit[j];
	            optionText = optionText.replace("\n","");
	            anOption = new Option(optionText,optionText);
	            if ( "html" == optionText ) anOption.setAttribute("selected","selected");
		        display.add(anOption);
	        }
        }
		notify(res);
    }    
...
function SensorSiteAdmin_StartWidgetLoading(res) {
    sensorSiteadmin.loadStartWidget(res);
}

function SensorSiteAdmin_AllWidgetLoading(res) {
    sensorSiteadmin.loadAllWidgets(res);
}

/**
* Singleton instance for the Site Admin sensor
*/
var sensorSiteadmin = new SensorSiteAdmin();var SensorLogin = new Class({
    initialize: function() {
    },

    loginRequest: function(uid,pass,tenant,container, onSucessF) {
	    this.uid = uid;
	    var data = new Object();
	    data.sensor = "sensor.login";
	    data.action = "LOGIN";
	    data.tenant = tenant;
	    data.user = uid;
	    data.passwd = pass;
        httpRequest.callAndNotify(PROCESS_XML, data, container, container, 
            sensorLogin_doLogin,onSucessF );
    },
    
    doLogin: function(response) {
    	var loginXml,loginInfo,fI, fldT,nodeName,roleI;

	    loginXml = loadXMLDoc(response.BODY);
	    loginInfo = loginXml.childNodes[0].childNodes[0];
	    fldT = loginInfo.childNodes.length;
	    this.baseCR = "none";
	    this.tenant = "none";
	    for ( fI=0; fI < fldT; fI++ ) {
		    nodeName = loginInfo.childNodes[fI].nodeName;
		    if ( "tenant" == nodeName )  
		        this.tenant = loginInfo.childNodes[fI].childNodes[0].nodeValue;
		    else  if ( "baseCnRole" == nodeName )  
		        this.baseCR = loginInfo.childNodes[fI].childNodes[0].nodeValue;
	    }
	    roleI = this.baseCR.indexOf(";");
	    this.role = this.baseCR.slice(roleI + 1);
	    this.page = this.tenant + "_" + this.role;
	    response.NOTIFY(response);
    }
});

function sensorLogin_doLogin(response) {
    sensorLogin.doLogin(response);
}

/**
 * This is a singleton instance for all the calls
 * Get the Role = this.role
 * Get the role specific start page = this.page
 * Get the baseCR = this.baseCR
 * Get the user Id = this.uid
 * Move to the start page window.location.href = "admin.html";
 */
var sensorLogin = new SensorLogin();
